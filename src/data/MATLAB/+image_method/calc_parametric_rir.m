...
for i=1:2:length(varargin)
    ...
    switch lower(name)
        ...
        case {'energythresh','energythreshold'}
            energyThresh = val;
        case {'amplitudethresh', 'amplitudethreshold', 'ampthresh', 'amplitudethreshold'}
            ampThresh = val;
        case 'zerofirstdelay'
            zeroFirstDelay = val;
        case 'maxwaves'
...
if ~isempty(ampThresh)
    ...
    for i=1:3
        if R_geomean(i)==0
            ...
            n = 1:log(ampThresh)/log(R_geomean(i));
            a = R_geomean(i).^n ./ (n.*roomDim(i) + L_diag);
            maxReflectionOrder_by_ampThresh(i) = ceil(find(a>=ampThresh, 1, 'last')/2)*2;
        end
    end
end
...
[reflections.amp, reflections.relativePos, reflections.r] = image_method.calc_reflection_amp(arrayPos, reflectionsPos, reflections.hits, (R+1)./(1-R), angleDependence);
